...
export interface UserPackage {
  pkg: string;
  /** "YYYY-MM-DD HH:MM:SS" from dumpsys, or "" when unknown. */
  updated: string;
}

/**
 * User-installed (`-3`) packages, **newest install/update first** so the app you
 * just sideloaded is at the top of the picker. Install time comes from
 * `dumpsys package`'s `lastUpdateTime` (best-effort; falls back to name order).
 */
export async function listUserPackages(adbPath: string, serial: string): Promise<UserPackage[]> {
  const [list, times] = await Promise.all([
    execAdb(adbPath, ["-s", serial, "shell", "pm", "list", "packages", "-3"], 15000, serial),
    execAdb(
      adbPath,
      ["-s", serial, "shell", "dumpsys package packages | grep -E 'Package \\[|lastUpdateTime='"],
      20000,
      serial
    ).catch(() => ({ stdout: "", stderr: "" })),
  ]);
  ...
  const updatedOf = new Map<string, string>();
  let cur = "";
  for (const raw of times.stdout.split(/\r?\n/)) {
    const line = raw.trim();
    const h = line.match(/^Package \[([^\]]+)\]/);
    if (h) {
      cur = h[1];
      continue;
    }
    const t = line.match(/^lastUpdateTime=(.+)$/);
    if (t && cur) {
      updatedOf.set(cur, t[1].trim());
    }
  }
  ...
  return [...userPkgs]
    .map((pkg) => ({ pkg, updated: updatedOf.get(pkg) ?? "" }))
    .sort((a, b) => ms(b.updated) - ms(a.updated) || a.pkg.localeCompare(b.pkg));
}
