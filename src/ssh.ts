...
export interface InstalledApp {
  /** Full path to the `.app` bundle on the device. */
  path: string;
  /** `.app` folder name without the extension. */
  name: string;
  /** CFBundleIdentifier, when `plutil` could read it (else ""). */
  bundleId: string;
}

/**
 * Lists user apps installed on a (jailbroken) iOS device — the `.app` bundles
 * under /var/containers/Bundle/Application. System apps (/Applications) are left
 * out since you can't repackage those into a useful IPA.
 */
export async function listInstalledApps(target: SshTarget): Promise<InstalledApp[]> {
  const script =
    'for a in /var/containers/Bundle/Application/*/*.app; do ' +
    '[ -d "$a" ] || continue; ' +
    'n=$(basename "$a" .app); ' +
    "id=$(plutil -p \"$a/Info.plist\" 2>/dev/null | grep -m1 '\"CFBundleIdentifier\"' | sed 's/.*=> \"//; s/\".*//'); " +
    'printf \'%s\\t%s\\t%s\\n\' "$a" "$n" "$id"; ' +
    "done";
  const { stdout } = await execSsh(target, script, 20000);
  return stdout
    .split(/\r?\n/)
    .map((l) => l.split("\t"))
    .filter((c) => c[0] && c[0].endsWith(".app"))
    .map((c) => ({ path: c[0], name: c[1] || c[0].split("/").pop() || "app", bundleId: c[2] || "" }))
    .sort((a, b) => a.name.localeCompare(b.name));
}

/**
 * Repackages a **non-encrypted** app bundle (sideloaded / dev-signed) into an
 * .ipa and downloads it. App Store apps come out with a still-encrypted binary —
 * those need a runtime decryptor (frida-ios-dump / bagbak), not this.
 */
export async function extractSideloadedIpa(
  target: SshTarget,
  appPath: string,
  localPath: string,
  onOutput: (line: string) => void = () => {}
  ...
  const q = shq(appPath);
  const check = await execSsh(target, "command -v zip || echo MISSING", 8000);
  if (/MISSING/.test(check.stdout) || !check.stdout.trim()) {
    throw new SshError(t("ssh.err.zipMissing"));
  }
  const remoteIpa = "/tmp/frivenv-extract.ipa";
  const stage = "/tmp/frivenv-ipa-$$";
  const build =
    `rm -rf ${stage} ${remoteIpa} && mkdir -p ${stage}/Payload && ` +
    `cp -R ${q} ${stage}/Payload/ && cd ${stage} && zip -qr ${remoteIpa} Payload && ` +
    `cd / && rm -rf ${stage}`;
  await execLog(target, build, onOutput, { timeout: 120000 });
  onOutput("# downloading " + remoteIpa);
  await sftpDownload(target, remoteIpa, localPath, 300000);
  await execSsh(target, `rm -f ${remoteIpa}`, 8000).catch(() => {});
}
...
/**
 * Uploads a local file to a root-owned path on the device (mkdir -p the parent,
 * chmod `mode`). As root that's a direct SFTP; as a non-root SSH user (rootless
 * jailbreak), it lands in $HOME first and gets moved into place with sudo, same
 * trick as killFridaServer.
 */
async function pushRemoteFile(
  ...
  if (isRoot) {
    ...
    onOutput("# upload " + baseName(localPath) + " -> " + remotePath);
    await sftpUpload(target, localPath, remotePath, 120000);
    await execLog(target, `chmod ${mode} ${q(remotePath)}`, onOutput, { timeout: 10000 });
    return;
  }
  ...
  // denied". Land the file in the user's home first, then move it into place
  // with sudo (same trick as killFridaServer).
  const stageName = `frivenv-${remotePath.slice(remotePath.lastIndexOf("/") + 1)}`;
  onOutput("# upload " + baseName(localPath) + " -> " + remotePath);
  await sftpUpload(target, localPath, stageName, 120000); // relative path -> lands in $HOME
  const { stdout } = await execSsh(target, `printf '%s' "$HOME/${stageName}"`, 8000);
  const staged = stdout.trim() || `/var/mobile/${stageName}`;

  const move = rootRun(
    target,
    `mkdir -p ${q(remoteDir)} && mv ${q(staged)} ${q(remotePath)} && chmod ${mode} ${q(remotePath)}`
  );
...
export async function pushFridaServerBinary(
  ...
  await pushRemoteFile(target, localBinaryPath, remotePath, "755", onOutput);
}

/**
 * The directory frida-server looks in for its `frida-agent.dylib` sidecar (only
 * needed for spawn — `frida -f`, which injects before the app's own code runs;
 * attach doesn't need it). Same rootful/rootless split as the server folder.
 */
export function iosFridaAgentDir(remoteServerDir: string): string {
  return remoteServerDir.startsWith("/var/jb") ? "/var/jb/usr/lib/frida" : "/usr/lib/frida";
}

/**
 * Pushes frida-agent.dylib next to the server so `frida -f` (spawn) works, not
 * just attach. Safe to call even when the .deb didn't ship one (caller checks).
 */
export async function pushFridaAgent(
  target: SshTarget,
  localAgentPath: string,
  remoteServerDir: string,
  onOutput: (line: string) => void
): Promise<void> {
  const remotePath = `${iosFridaAgentDir(remoteServerDir)}/frida-agent.dylib`;
  await pushRemoteFile(target, localAgentPath, remotePath, "644", onOutput);
}
