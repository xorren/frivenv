...
interface ProfileRow {
  name: string;
  displayName: string;
  /** What's actually installed in the venv right now ("" = not installed / can't read). */
  pythonVersion: string;
  fridaVersion: string;
  fridaToolsVersion: string;
  /** What the profile asked for at creation (profiles.json). */
  reqPython: string;
  reqFrida: string;
  reqTools: string;
  venvDir: string;
...
export class ManageProfilesPanel {
  ...
  private sendProfiles(): void {
    const rows: ProfileRow[] = loadProfiles(this.dataDir).map((p) => {
      ...
      // The real versions come from the venv (pyvenv.cfg + dist-info folders), not
      // from what was requested — a pip install can fail (no wheel for that
      // python) and still leave a folder behind.
      const info = venvExists ? readVenvInfo(venvDir) : null;
      return {
        name: p.name,
        displayName: displayNameOf(p),
        pythonVersion: info?.pythonVersion ?? "",
        fridaVersion: info?.fridaVersion ?? "",
        fridaToolsVersion: info?.fridaToolsVersion ?? "",
        reqPython: p.python_version,
        reqFrida: p.frida_version,
        reqTools: p.frida_tools_version,
        venvDir,
        venvExists,
      };
    });
...
${PANEL_KIT_JS}
  ...
  function renderProfiles() {
    ...
    rows.forEach((row, i) => {
      ...
      // Per-package: show the version actually installed in the venv. If it's
      // missing or doesn't match what the profile asked for, say so — a pip
      // install can fail (no wheel for that Python) and still leave a folder.
      const part = (label, actual, req) => {
        if (!row.venvExists) return label + ' ' + (req || '?');
        if (actual && (!req || actual === req)) return label + ' ' + actual;
        if (actual) return label + ' ' + actual + ' ' + L['nw.ver.requested'].replace('{0}', req);
        return label + ' ' + L['nw.ver.notInstalled'].replace('{0}', req || '?');
      };
      const mismatch =
        row.venvExists &&
        ((row.reqFrida && row.fridaVersion !== row.reqFrida) ||
          (row.reqTools && row.fridaToolsVersion !== row.reqTools) ||
          (row.reqPython && row.pythonVersion && row.pythonVersion !== row.reqPython));
      const pill = !row.venvExists
        ? pkPill('miss', L['nw.pill.venvMissing'])
        : mismatch
          ? pkPill('warn', L['nw.pill.venvMismatch'])
          : pkPill('ok', 'venv');
      ...
      if (mismatch) ver.style.color = 'var(--vscode-editorWarning-foreground, #cca700)';
      ver.textContent = [
        part('py', row.pythonVersion, row.reqPython),
        part('frida', row.fridaVersion, row.reqFrida),
        part('tools', row.fridaToolsVersion, row.reqTools),
      ].join(' / ');
