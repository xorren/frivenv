  ...
  private async handleMessage(msg: WebviewToHostMessage): Promise<void> {
    switch (msg.type) {
      ...
      case "pullApk": {
        ...
        // Newest install/update first (adb.listUserPackages sorts), with the date
        // shown — the app you just sideloaded is the top entry.
        const pick = await vscode.window.showQuickPick(
          packages.map((p) => ({
            label: p.pkg,
            description: p.updated ? t("dw.apkPull.installedAt", p.updated) : "",
          })),
          {
